...
const HeaderCenter = ({ wishlistItem, cartSlice }: any) => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_DOMAIN}/categories`);
        const data = await res.json();
        if (data.success && data.data?.length > 0) {
          setCategories(data.data);
          setSelectedItem(data.data[0]); // default to first category
        }
      } catch (err) {
  ...
  const cartCount = mounted ? cartSlice.length : 0;
  ...
  const handleMenuClick = (info: any) => {
    const found = categories.find((cat) => String(cat.id) === String(info.key));
  ...
  const handleLogout = async () => {
    try {
      ...
      const token = loginUser?.token || loginUser?.access_token || null;

      await fetch(`${process.env.NEXT_PUBLIC_DOMAIN}/auth/logout`, {
        method: "POST",
        headers: {
