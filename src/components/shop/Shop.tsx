...
const Shop = ({ col, lg }: any) => {
  ...
  const postData = useMemo(() => {
    ...
    if (categoryFromUrl) payload.category = categoryFromUrl;
    if (selectedCategory?.length > 0) payload.category = selectedCategory;
    if (sortOption) payload.sort = sortOption;
  ...
  // 1️⃣ URL build kore query parameters add korbo
  ...
  // 2️⃣ Full URL
  const apiUrl = `https://backend.zhenaura.net/api/v2/products/search?${queryParams.toString()}`;
                      ...
                      <Col className="col-6">
                        <div className="bb-bl-btn">
                          <button
                            ...
                            type="button"
                            className={`grid-btn btn-grid-100 ${!isGridView ? "active" : ""}`}
                          >
